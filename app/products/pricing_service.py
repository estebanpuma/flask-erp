...
from .models import ProductDesign, ProductVariantMaterialDetail, ProductPriceHistory
from ..materials.models import Material, MaterialLot
...
class PricingService:
    ...
    def _gather_costs(self, design: ProductDesign):
        ...
        subq = (
            ...
            .join(MaterialLot, MaterialLot.material_id == ProductVariantMaterialDetail.material_id)
            .filter(
        ...
        total_material_cost = session.query(func.coalesce(func.sum(subq.c.num / subq.c.den), 0)).scalar()

        # 4) Mano de obra total
        total_labor_hours = sum(Decimal(str(v.standar_time or 0)) for v in design.variants)

        return Decimal(str(total_material_cost)), total_labor_hours

    def calculate_price(self, design_id: int, override_markup_pct=None, include_tax=False, force_recalc=False):
        ...
        entity = PricingEntity(mat_cost, labor_h, self.labor_rate, self.overhead_rate, markup)
