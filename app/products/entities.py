...
class ProductEntity:
    ...
    def __init__(self, data: dict):
        ...
        self.designs = [
            ProductDesignEntity
        ]  # para adjuntar diseños si se desea construir como agregado
...
class ProductDesignEntity:
    ...
    def __init__(self, data: dict):
        ...
        # Service
        self.product_code = parse_str(
            data.get("product_code"), field="product_code"
        )  # Proveído por el servicio
...
class ProductVariantEntity:
    def __init__(self, data: dict):
        ...
        # Service
        self.design_code = parse_str(
            data.get("design_code")
        ).upper()  # Proveído por el servicio
    ...
    def _validate(self):
        ...
        if not isinstance(self.materials, list) or not self.materials:
            raise ValidationError(
                "Cada variante debe tener al menos una lista de materiales asociada."
            )
...
class ProductVariantMaterialEntity:
    ...
    def _validate(self):
        if self.quantity <= 0:
            raise ValidationError(
                "No se pude ingresar una cantidad menor o igual a cero"
            )
