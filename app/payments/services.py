from .models import PaymentMethod, PaymentTransaction, PaymentAgreement
...
from .dto import PaymentTransactionCreateDTO, PaymentTransactionUpdateDTO, PaymentAgreementCreateDTO, PaymentAgreementUpdateDTO
...
class PaymentMethodService:
    ...
    def delete_obj(method):
        transaction_exist = PaymentTransaction.query.filter(PaymentTransaction.method_id == method.id).first()
        if transaction_exist:
            raise ValueError('No se puede eliminar el metodo porque esta asociado a un pago')
...
class PaymentAgreementService:
    ...
    def create_payment_agreement(dto:PaymentAgreementCreateDTO):
        ...
        if order.status in ("Cancelada", "Cerrada", "Aprobada"):
            raise AppError("No se pueden registrar acuerdos en órdenes cerradas o aprobadas.")

        # Validar que el nuevo acuerdo no exceda el total de la orden
        total_agreements = sum(
            a.amount for a in order.agreements
        )
        new_total = total_agreements + dto.amount
        if new_total > order.total:
            raise AppError(f"El monto total de las cuotas comprometidas excede el total de la orden. Monto maximo:{str(order.total-total_agreements)}")
    ...
    def patch_obj(id: int, dto) -> PaymentAgreement:
        ...
        if order.status in ("Cancelada", "Cerrada"):
            raise AppError("No se pueden modificar acuerdos en órdenes aprobadas o cerradas.")
        ...
        if dto.amount:
            ...
            new_total = total_agreements + dto.amount
            if new_total > order.total:
                raise AppError("El nuevo monto total de cuotas comprometidas excede el total de la orden.")
    ...
    def delete_obj(agreement):
        ...
        if order.status in ("canceled", "closed", "approved"):
            raise AppError("No se pueden eliminar acuerdos de órdenes aprobadas o cerradas.")
        try:
...
class PaymentTransactionService:
    ...
    def create_payment_transaction(dto):
        ...
        if not order:
            raise NotFoundError("La orden de venta no existe.")
        if order.status in ("Cancelada", "Cerrada"):
            raise AppError("No se pueden registrar pagos en órdenes canceladas o cerradas.")
    ...
    def patch_obj(payment:PaymentTransaction, dto) -> PaymentTransaction:
        ...
        if order.status in ("Canceladas", "Cerradas"):
            raise AppError("No se pueden modificar pagos en órdenes canceladas o cerradas.")
    ...
    def delete_obj(payment:PaymentTransaction):
        ...
        if order.status in ("canceled", "closed"):
            raise AppError("No se pueden eliminar pagos en órdenes canceladas o cerradas.")
    ...
    def get_obj_list(filters: dict = None) -> list[PaymentTransaction]:
        ...
        if "from_date" in filters and "to_date" in filters:
            query = query.filter(
                PaymentTransaction.payment_date.between(filters["from_date"], filters["to_date"])
            )
            return query.all()
        else:
