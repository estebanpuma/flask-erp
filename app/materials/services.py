...
class MaterialGroupServices:
    ...
    def update(instance, data):
        ...
        if 'name' in data:
            ...
            existing = MaterialGroup.query.filter_by(name=name, code=instance.code).first()
            if existing and instance:
                raise ConflictError("Ya existe un grupo con ese nombre para el codigo seleccionado.")
    ...
    def get_list(filters=None):
        ...
        if filters:
            ...
            if 'material_group_id' in filters:
                query = query.filter(Material.material_group_id == int(filters['material_group_id']))
