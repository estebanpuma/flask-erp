...
from .product_lot_schemas import product_lot_fields, product_lot_movement_fields
from .product_lot_services import ProductLotMovementService, ProductLotService


class ProductLotGetResource(BaseGetResource):
    schema_get = staticmethod(ProductLotService.get_obj)
    schema_list = staticmethod(
        lambda: ProductLotService.get_obj_list(request.args.to_dict())
    )
...
class ProductLotMovementGetResource(BaseGetResource):
    ...
    schema_list = staticmethod(
        lambda: ProductLotMovementService.get_obj_list(request.args.to_dict())
    )
    output_fields = product_lot_movement_fields
