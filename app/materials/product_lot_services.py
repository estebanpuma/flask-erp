...
from .dto.product_lot_movement_dto import ProductLotAdjustmentDTO, ProductLotMovementCreateDTO
...
class ProductLotService:
    ...
    def create(dto: ProductLotCreateDTO) -> ProductLot:
        ...
        lot = ProductLot(
            ...
            status=ProductStock.IN_STOCK.value  # Estado inicial
        )
        db.session.add(lot)
    ...
    def adjust_obj(data:dict):
        with db.session.begin():
            ...
            lot = ProductLotService.adjust_to_real(dto)
            return lot

    @staticmethod
    def adjust_to_real(dto: ProductLotAdjustmentDTO) -> ProductLot:
        lot = ProductLotService.get_obj(dto.product_lot_id)
...
class ProductLotMovementService:
    ...
    @staticmethod
    def create_obj(data:dict)->ProductLotMovement:
        with db.session.begin():
            dto = ProductLotMovementCreateDTO(**data)
            movement = ProductLotMovementService.create(dto)
            return movement

    @staticmethod
    def create(dto: ProductLotMovementCreateDTO) -> ProductLotMovement:
        lot = db.session.get(ProductLot, dto.product_lot_id)
        if not lot:
            raise NotFoundError(f"ProductLot con id {dto.product_lot_id} no encontrado.")

        # Validar cantidad positiva
        if dto.quantity <= 0:
            raise ValidationError("La cantidad debe ser mayor a cero.")

        if dto.movement_type == 'IN':
            # Entrada ➜ sumamos al stock del lote
            lot.quantity += dto.quantity

        elif dto.movement_type == 'OUT':
            # Salida ➜ verificamos stock suficiente
            ...
            raise ValidationError(f"Tipo de movimiento no permitido: {dto.movement_type}")
