...
from .dto.inventory_movements_dto import InventoryMovementAdjustDTO, InventoryMovementOutDTO, InventoryMovementTransferDTO
...
class InventoryMovementService:
    ...
    def create_obj_out(data:dict):
        with db.session.begin():
            ...
            movement = InventoryMovementService.create_out(dto.lot_id, dto.quantity, dto.note)
            return movement
    ...
    def create_obj_transfer(data:dict):
        with db.session.begin():
            ...
            movement = InventoryMovementService.transfer_material_lot(lot_id=dto.lot_id,
    ...
    def adjust_obj(data:dict) -> InventoryMovement:
        with db.session.begin():
            ...
            adjusment = InventoryMovementService.adjust_to_real(lot_id=dto.lot_id,
                                                                real_quantity=dto.quantity,
                                                                note=dto.note)
            return adjusment
    ...
    @staticmethod
    def transfer_material_lot(lot_id: int, destination_warehouse_id: int, note: str = None ) -> InventoryMovement:
        lot = db.session.get(MaterialLot, lot_id)
        if not lot:
            raise NotFoundError(f"MaterialLot con id {lot_id} no encontrado.")

        if lot.warehouse_id == destination_warehouse_id:
            raise ValidationError("La bodega de destino debe ser diferente a la de origen.")
        ...
        if lot.quantity_committed > 0:
            raise ValidationError("No se puede transferir un lote con reservas comprometidas.")
        ...
        movement = InventoryMovement(
            ...
            note=note or f"Transferencia de bodega {old_warehouse_id} a {destination_warehouse_id}",
            date=datetime.today()
        )
        db.session.add(movement)
    ...
    @staticmethod
    def adjust_to_real(lot_id: int, real_quantity: float, note: str = None) -> InventoryMovement:
        lot = db.session.get(MaterialLot, lot_id)
        if not lot:
            raise NotFoundError(f"MaterialLot con id {lot_id} no encontrado.")
