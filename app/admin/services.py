...
class AdminServices:
    ...
    def create_user(username, ci, email, password, job_code=None, phone=None):
        ...
        user = user_class(
            username=username, ci=ci, email=email, phone=phone, job_code=job_code
        )
        user.set_password(password)
        job = Job.query.filter(Job.code == job_code).first()
        role = AdminServices.create_role(
            code=job.code, name=job.name, description=job.description
        )
        user.roles = [role]
        try:
    ...
    def initialize_admin_user():
        ...
        admin_role = AdminServices.create_role(
    ...
    def update_roles(user_id, roles):
        ...
        try:
            db.session.commit()
            current_app.logger.info("Role updated successfully")
        except Exception as e:
            db.session.rollback()
...
class WorkerService:
        ...
        if job_id is not None:
            ...
            if not job:
                raise ValidationError(
                    f"No existe un puesto de trabajo con ID: {job_id}"
                )
    ...
    def update_worker(worker: Worker, dto) -> Worker:
        ...
        if dto.job_id is not None:
            ...
            if not job:
                raise ValidationError(
                    f"No existe un puesto de trabajo con ID: {dto.job_id}"
                )
...
class JobService:
    ...
    def create_job(code: str, name: str, description: str = None) -> Job:
        ...
        if job:
            raise ValidationError(
                f"Ya existe un puesto de trabajo con el codigo:{code}. Puesto:{job.name}"
            )
        job = Job.query.filter(Job.name == name).first()
        if job:
            raise ValidationError(
                f"Ya existe un puesto de trabajo con el nombre:{job.name}"
            )

        new_job = Job(code=code, name=name, description=description)
    ...
    def search_job(q: str, limit=10) -> list[Job]:
        query = (
            ...
            .filter((Job.code.ilike(f"%{q}%")) | (Job.description.ilike(f"%{q}%")))
            .limit(limit)
