...
user_roles = db.Table(
    ...
    db.Column(
        "user_ci",
        db.String(),
        db.ForeignKey("users.ci", ondelete="CASCADE", onupdate="CASCADE"),
        primary_key=True,
    ),
    db.Column(
        "role_code",
        db.String(),
        db.ForeignKey("roles.code", ondelete="CASCADE", onupdate="CASCADE"),
        primary_key=True,
    ),
)
...
class Job(db.Model, SoftDeleteMixin):
    ...
    def count_users_job(self):
        user_job = (
            User.query.join(Job).filter(Job.code == self.code, User.is_active).all()
        )
        return len(user_job)
    ...
    def count_workers_job(self):
        worker_job = (
            Worker.query.join(Job).filter(Job.code == self.code, Worker.is_active).all()
        )
...
class Worker(BaseModel, SoftDeleteMixin):
    ...
    def calculate_hour_rate(self):
        calculated_hour_rate = (
            self.salary / 173.2
        )  # 40 horas/semana × 4.33 semanas/mes = 173.2 horas/mes (4.33 es el promedio de semanas por mes: 52 semanas/año ÷ 12 meses)
