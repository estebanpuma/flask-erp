...
workers_bp = Blueprint(
    "workers", __name__, url_prefix="/workers", template_folder="../templates"
)
...
def workers_edit(id):
    from ...admin.services import AdminServices, WorkerService

    worker = WorkerService.get_obj(id)
    jobs = AdminServices.get_all_jobs()
    return render_template(
        "workers/workers_edit.html", worker_id=id, worker=worker, jobs=jobs
    )
