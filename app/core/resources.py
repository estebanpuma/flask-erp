...
from .error_handlers import AppError, ConflictError, NotFoundError, ValidationError
        ...
        try:
            if resource_id:
                ...
                if not instance:
                    ...
                    )  # usamos excepción personalizada
            ...
            return error_response(
                str(e), 404
            )  # puedes dejarlo aquí o usar handler global
...
class BasePostResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            payload = (
                marshal(instance, self.output_fields)
            ...
            if isinstance(e.orig, psycopg2.errors.UniqueViolation):
                constraint = (
                    ...
                    if e.orig.diag.constraint_name
                    else "violación de unicidad"
                )
                return error_response(
                    f"El recurso ya existe o viola una restricción de unicidad: {constraint}",
                    409,
                )
                ...
                constraint = (
                    e.orig.diag.constraint_name
                    if e.orig.diag.constraint_name
                    else "violación de clave foránea"
                )
                return error_response(
                    f"Error de referencia: No se pudo crear el recurso debido a una restricción de clave foránea: {constraint}",
                    400,
                )
            return error_response(
                f"Error de integridad en la base de datos: {str(e)}", 400
            )
        ...
        try:
            ...
            if deleted:
                return success_response(
                    {"message": "Recurso eliminado correctamente"}, 200
                )
            else:
...
class BasePatchResource(Resource):
    ...
    def patch(self, resource_id):
        try:
            ...
            response = (
                marshal(updated, self.output_fields) if self.output_fields else updated
            )
    ...
    def post(self):
        try:
            if "file" not in request.files:
                return error_response(
                    "No se encontró archivo Excel con clave 'file'.", 400
                )
            ...
            if not self.import_service or not self.row_handler:
                raise ValueError(
                    "Faltan componentes en la clase hija: 'import_service' y/o 'row_handler'."
                )
