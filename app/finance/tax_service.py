...
class TaxService:
    ...
    def _get_rate(self, code: str = None, on_date: date = None) -> Decimal:
        ...
        q = (
            self.session.query(TaxRate.rate)
            .filter(
                TaxRate.is_active == True,
                TaxRate.effective_date <= on
            )
        )
        if code:
    ...
    def calculate_tax(self, net_amount: Decimal, code: str = None, on_date: date = None) -> Decimal:
        rate = self._get_rate(code, on_date)
        return (net_amount * rate).quantize(Decimal('0.01'))

    def calculate_gross(self, net_amount: Decimal, code: str = None, on_date: date = None) -> Decimal:
