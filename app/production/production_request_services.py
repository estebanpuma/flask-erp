...
from ..core.exceptions import ValidationError, NotFoundError, AppError, ConflictError
...
class ProductionRequestServices:
    ...
    @staticmethod
    def create_production_request(origin_type:str, 
                                  origin_id:int,
                                  title:str,
                                  purpose:str=None,
                                  notes:str=None):
        ...
        if not ProductionSourceTypeEnum.has_value(origin_type):
            raise ValidationError(f'El origen del requerimiento: {origin_type} no es valido')
        ...
        if origin_type == ProductionSourceTypeEnum.SALES.value:
            ...
            if not sale:
                raise ValidationError(f'No existe una orden de venta con el id: {origin_id}')
        ...
        new_request = ProductionRequest(origin_type=origin_type,
                                        origin_id=origin_id,
                                        title=title,
                                        purpose=purpose,
                                        notes=notes)
