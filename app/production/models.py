...
production_order_requests = db.Table(
    ...
    db.Column(
        "production_request_id", db.Integer, db.ForeignKey("production_requests.id")
    ),
)
...
class ProductionOrder(db.Model):
    ...
    workers_assigned = db.Column(
        db.Integer, default=1
    )  # Total trabajadores en la orden
    total_overtime_hours = db.Column(
        db.Float, default=0.0
    )  # Horas extra global para todos los trabajadores

    lines = db.relationship(
        "ProductionOrderLine", backref="production_order", cascade="all, delete-orphan"
    )
    material_summaries = db.relationship(
        "ProductionMaterialSummary",
        backref="production_order",
        cascade="all, delete-orphan",
    )
    events = db.relationship(
        "ProductionEvent", backref="production_order", cascade="all, delete-orphan"
    )
    plan_change_logs = db.relationship(
        "PlanChangeLog", backref="production_order", cascade="all, delete-orphan"
    )
    ...
    product_lots = db.relationship(
        "ProductLot", back_populates="production_order", lazy="dynamic"
    )
...
class ProductionOrderLine(db.Model):
    ...
    checkpoints = db.relationship(
        "ProductionCheckpoint", backref="order_line", cascade="all, delete-orphan"
    )
    materials = db.relationship(
        "ProductionMaterialDetail", backref="order_line", cascade="all, delete-orphan"
    )
...
class ProductionLineWorkLog(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True)
    production_order_line_id = db.Column(
        db.Integer, db.ForeignKey("production_order_lines.id"), nullable=False
    )
    ...
    entries = db.relationship(
        "WorkLogEntry",
        back_populates="production_line_work_log",
        cascade="all, delete-orphan",
    )
...
class WorkLogEntry(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True)
    production_line_work_log_id = db.Column(
        db.Integer, db.ForeignKey("production_line_work_logs.id"), nullable=False
    )
    ...
    production_line_work_log = db.relationship(
        "ProductionLineWorkLog", back_populates="entries"
    )
...
class ProductionRequest(BaseModel):
    ...
    lines = db.relationship(
        "ProductionRequestLine", back_populates="request", cascade="all, delete-orphan"
    )
    ...
    @hybrid_property
    def total_time(self):
        return sum(line.quantity * line.variant.standar_time for line in self.lines)

    @hybrid_property
    def total_cost(self):
        return sum(line.quantity * line.variant.unit_cost for line in self.lines)
...
class ProductionRequestLine(BaseModel):
    ...
    request_id = db.Column(
        db.Integer, db.ForeignKey("production_requests.id"), nullable=False
    )
    # Para trazabilidad:  ↴
...
class ProductionMaterialSummary(BaseModel):
    ...
    quantity_reserved = db.Column(db.Float, default=0.0)
    quantity_pending = db.Column(
        db.Float, default=0.0
    )  # lo que falta reservar (útil para sugerir compras)
...
class ProductionRework(db.Model):
    ...
    checkpoint_id = db.Column(
        db.Integer, db.ForeignKey("production_checkpoints.id"), nullable=False
    )
    ...
    rework_materials = db.relationship(
...
class ProductionMaterialDetailForRework(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    rework_id = db.Column(
        db.Integer, db.ForeignKey("production_reworks.id"), nullable=False
    )
    material_id = db.Column(db.Integer, db.ForeignKey("materials.id"), nullable=False)
...
class ProductionEvent(db.Model):
    ...
    event_type = db.Column(
        db.String(50)
    )  # Ej: 'material_shortage', 'machine_breakdown'
...
class Operations(BaseModel, SoftDeleteMixin):
    ...
    variant_operations = db.relationship(
        "ProductVariantOperationDetail", back_populates="operation"
    )
...
class OperationRouting(BaseModel):
    __tablename__ = "operation_routing"
    id = db.Column(db.Integer, primary_key=True)
    model_id = db.Column(db.Integer, nullable=False, unique=True, index=True)
    lt = db.Column(db.Numeric(10, 3), nullable=False, default=Decimal("0"))
    bottleneck_node_key = db.Column(db.String(64), nullable=True)


class OperationRoutingNode(db.Model):
    __tablename__ = "operation_routing_node"
    id = db.Column(db.Integer, primary_key=True)
    routing_id = db.Column(
        db.Integer,
        db.ForeignKey("operation_routing.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )

    node_key = db.Column(db.String(64), nullable=False)  # "n1" etc. (único por routing)
    operation_id = db.Column(db.Integer, nullable=False)

    ct = db.Column(db.Numeric(10, 3), nullable=False)
    es = db.Column(db.Numeric(10, 3), nullable=False)
    ef = db.Column(db.Numeric(10, 3), nullable=False)
    ls = db.Column(db.Numeric(10, 3), nullable=False)
    lf = db.Column(db.Numeric(10, 3), nullable=False)
    slack = db.Column(db.Numeric(10, 3), nullable=False)
    crit = db.Column(db.Boolean, nullable=False, default=False)
    level = db.Column(db.Integer, nullable=False, default=0)

    __table_args__ = (
        db.UniqueConstraint("routing_id", "node_key", name="uq_routing_node_key"),
    )


class OperationRoutingEdge(db.Model):
    __tablename__ = "operation_routing_edge"
    id = db.Column(db.Integer, primary_key=True)
    routing_id = db.Column(
        db.Integer,
        db.ForeignKey("operation_routing.id", ondelete="CASCADE"),
    ...
    from_node_id = db.Column(
        db.Integer,
        db.ForeignKey("operation_routing_node.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "routing_id", "from_node_id", "to_node_id", name="uq_routing_edge"
        ),
    )


class OperationRoutingStep(db.Model):
    __tablename__ = "operation_routing_step"
    id = db.Column(db.Integer, primary_key=True)
    routing_id = db.Column(
        db.Integer,
        db.ForeignKey("operation_routing.id", ondelete="CASCADE"),
    ...
    parallel_with_prev = db.Column(db.Boolean, nullable=False, default=False)

    __table_args__ = (
        db.UniqueConstraint("routing_id", "sequence", name="uq_routing_step_seq"),
    )
...
class ProductionResource(BaseModel, SoftDeleteMixin):
    ...
    efficiency = db.Column(db.Numeric(4, 2), nullable=True, default=1)  # not now
    ...
    variant_operation_resource = db.relationship(
        "VariantOperationResource", back_populates="resource"
    )
...
class VariantOperationResource(db.Model):  # nuevo
    ...
    resource_id = db.Column(
        db.Integer, db.ForeignKey("production_resources.id"), nullable=False
    )

    res_min = db.Column(
        db.Numeric(10, 4), nullable=False, default=0
    )  # min/u que consume este recurso
    qty_required = db.Column(
        db.Numeric(10, 4), nullable=False, default=1
    )  # si pides 2 operarios, etc.
    role = db.Column(db.String(20))

    resource = db.relationship(
        "ProductionResource", back_populates="variant_operation_resource"
    )
    variant_operation = db.relationship(
        "ProductVariantOperationDetail", back_populates="resources"
    )
