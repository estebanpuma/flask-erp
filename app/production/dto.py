...
class ProductionOrderCreateDTO(BaseModel):
    production_request_ids: List[int] = Field(
        min_length=1, description="Debe ingresar un requerimiento de produccion"
    )
    start_date: date
...
class ProductionOrderUpdatePlanDTO(BaseModel):
    workers_assigned: Optional[int] = Field(
        None,
        gt=0,
        description="Número de trabajadores asignados (opcional, debe ser > 0 si está presente)",
    )
    hours_per_shift: Optional[float] = Field(
        8,
        gt=0,
        lt=24,
        description="las horas por turno no pueden ser menores de 0 o mayores de 24",
    )
    overtime_hours: Optional[float] = Field(
        0, ge=0, description="Horas extra no pueden ser menores que 0"
    )
